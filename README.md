...
- 快速淘汰已经失效的代理
- 快速淘汰端口不通的代理
- 把最基础的“能否连上”从纯模拟改成真实测试

注意：

这还不是“代理协议真的能转发请求”的完整证明。  
它只说明：**这个地址端口当前能建立 TCP 连接。**
...
所以示例版抓取阶段可以到 200 条，但客户端默认先跑 `50` 条，避免第一次体验太慢。  
等你下一步要接数据库和批量并发时，再把这个上限改大，或者改成分页 / 分批流程。

## 下一步你可以怎么扩展

1. 把 L1 改成真实 HTTP 代理请求测试
2. 把 L2 改成真实 HTTPS CONNECT 测试
